...
SEARCH_FIELDS = ["Plastic Type", "Size", "Catalog Number", "Supplier"]
...
for i, field in enumerate(SEARCH_FIELDS):
    ...
    unique_values = ['-- All Plastics --'] + sorted(df[field].dropna().astype(str).unique().tolist())
...
if st.button("Apply Search Filters"):
    if search_results.empty:
        ...
        st.success(f"🔍 Found **{len(search_results)}** matching plastic item(s):")
